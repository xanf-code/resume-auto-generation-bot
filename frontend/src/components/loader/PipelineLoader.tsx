      ...
      <div className="flex flex-col gap-6 sm:gap-8 px-4 sm:px-8 py-6 sm:py-10">
        <div className="flex items-start justify-between gap-3 sm:gap-4">
          <div className="flex flex-col gap-2 min-w-0">
            <span className="eyebrow">{aborting ? 'Stopping' : 'Generating'}</span>
            <h2 className="font-serif text-[22px] sm:text-[28px] leading-tight text-ink">
              {aborting
                ? 'Winding down this run…'
                : (job.humanLabel ?? 'Getting started…')}
            </h2>
            {aborting && (
              <p className="font-mono text-[12px] text-ink-soft max-w-md">
                Stop heard. The current step finishes first, then the press
                shuts down — usually a few seconds, sometimes a minute if a
                model call is mid-flight.
              </p>
            )}
          </div>
          <button
