"""Phase 7 - integration tests for run_job's Obsidian vault wiring.

Exercises the full seam: JD tagging always runs, retrieval + tuning resolution
run only when ``job.obsidian_learn`` is True, and the run note is always
written regardless. The pipeline itself is monkeypatched at the
``stream_pipeline`` boundary (the existing hermetic pattern from
tests/web/test_job_manager.py) so no LLM calls happen.
"""
...
def _seed_winning_backend_notes(vault_dir: Path) -> None:
    """Two past `interview` runs tagged `backend`, plus a threshold override."""
    for i in range(2):
        write_note(
...
def test_run_job_learn_on_seeds_examples_and_tuning_override(tmp_path, monkeypatch):
    ...
    with patch.object(runner_module.main_module, "stream_pipeline", side_effect=fake), \
         patch.object(runner_module, "classify_jd_type", return_value=["backend"]):
        run_job(job, manager)

    assert job.status == JobStatus.DONE
    assert job.jd_type == ["backend"]
    ...
    assert note.frontmatter["jd_type"] == ["backend"]
...
def test_run_job_learn_on_logs_retrieval_miss_to_activity_stream(tmp_path, monkeypatch):
    """Vault enabled, learning on, but nothing matches the tags - still logged."""
    monkeypatch.setenv("RESUME_VAULT_DIR", str(tmp_path))
...
def test_run_job_learn_on_no_vault_match_omits_diff_event(tmp_path, monkeypatch):
    """A tag with no by_tag override still runs learn-on, but logs no diff."""
    monkeypatch.setenv("RESUME_VAULT_DIR", str(tmp_path))
...
def test_run_job_learn_off_skips_retrieval_and_tuning_but_still_writes_note(tmp_path, monkeypatch):
    ...
    # Tags are still computed - the note always needs them - even with learning off.
    assert job.jd_type == ["backend"]
...
def test_run_job_disabled_vault_still_computes_tags_and_writes_no_note(tmp_path, monkeypatch):
