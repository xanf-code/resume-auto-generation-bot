...
BULLET_SHAPES: dict[str, dict[str, str]] = {
    "PAR": {
        ...
        "example": (
            "Cut support response from 48h to under 24h by adding Zendesk triage across a 12-person team"
        ),
    },
    "RESULT-FIRST": {
        "description": "lead with the number when it's strong enough to open cold.",
        "example": (
            "$1.2M new ARR generated by standing up a Northeast partner channel from zero"
        ),
    },
    "ACTION+STACK": {
        "description": "verb + tools + outcome. Use when the stack is the point.",
        "example": (
            "Built a Python/Airflow pipeline consolidating 4 reporting systems, dropping monthly close from 5 days to 2"
        ),
    },
    "CONTEXT-PAR": {
        "description": "add situation/scope when the scenario needs framing.",
        "example": (
            "Held inventory across 3 warehouses through a supply-chain disruption; cut carrying costs 15% via demand forecasting"
        ),
    },
}
